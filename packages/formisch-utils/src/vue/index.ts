/**
 * Vue adapter for formisch-utils.
 *
 * Combines `buildFormFields` + `generateInitialInput` with `@formisch/vue`'s
 * `useForm` composable into a single `useFormFields` composable.
 *
 * @requires @formisch/vue
 */

import { useForm } from "@formisch/vue";
import type { FormStore, DeepPartial } from "@formisch/vue";
...
export type { FormStore, SubmitHandler, DeepPartial } from "@formisch/vue";
...
/**
 * Vue composable that creates a Formisch form store together with the `FormFieldConfig`
 ...
 *     <template v-if="config.kind === 'object'">
 *       <DynamicField v-for="field in config.fields" :key="field.key" :of="form" :config="field" />
 *     </template>
 *   </Form>
 * </template>
 * ```
