 ...
 * @returns The reconstructed Valibot schema (may be async).
 */
function astNodeToSchemaAsync(ast: ASTNode, options?: ASTToSchemaAsyncOptions): GenericSchema | GenericSchemaAsync {
  if (ast.kind === 'validation' || ast.kind === 'transformation' || ast.kind === 'metadata') {
    throw new Error('Cannot convert standalone validation/transformation/metadata to schema. These must be part of a pipe.');
  }
 ...
 */
function buildBaseSchemaAsync(ast: ASTNode, options?: ASTToSchemaAsyncOptions): GenericSchema | GenericSchemaAsync {
  ...
  // Handle tuple
  if ((ast.type === 'tuple' || ast.type === 'loose_tuple' || ast.type === 'strict_tuple') && 'items' in ast) {
  ...
  if (ast.type === 'picklist' && 'options' in ast) {
    const options = ast.options.filter((opt): opt is string | number | bigint =>
  ...
  if (ast.type === 'record' && 'key' in ast && 'value' in ast) {
    return v.recordAsync(astNodeToSchemaAsync(ast.key, options) as any, astNodeToSchemaAsync(ast.value, options));
  }

  // Handle map
  if (ast.type === 'map' && 'key' in ast && 'value' in ast) {
    return v.mapAsync(astNodeToSchemaAsync(ast.key, options), astNodeToSchemaAsync(ast.value, options));
  }
  ...
  if (ast.type === 'instance' && 'class' in ast) {
    throw new Error(`Cannot reconstruct instance schema for class "${ast.class}". Instance schemas require runtime class references.`);
  }

  // Handle lazy
  if (ast.type === 'lazy') {
    throw new Error('Cannot reconstruct lazy schema from AST. Lazy schemas require runtime getter functions.');
  }
  ...
  switch (ast.type) {
    ...
    case 'blob': return v.blob();
    case 'symbol': return v.symbol();
    case 'any': return v.any();
    case 'unknown': return v.unknown();
    case 'never': return v.never();
    case 'nan': return v.nan();
    case 'null': return v.null_();
    case 'undefined': return v.undefined_();
    case 'void': return v.void_();
    case 'file': return v.file();
    case 'promise': return v.promise();
    default:
 ...
 */
function buildValidationAsync(ast: ASTNode & { kind: 'validation' }, options?: ASTToSchemaAsyncOptions): any {
  const { type, locales, requirement, message } = ast;

  // Check for custom validation
  if ('customKey' in ast && ast.customKey) {
    const customImpl = options?.validationDictionary?.get(ast.customKey);
    if (!customImpl) {
      throw new Error(
        `Custom validation '${ast.customKey}' referenced but not found in validation dictionary. ` +
        `Provide the implementation in options.validationDictionary.`
      );
    }
 ...
 */
function buildTransformationAsync(ast: ASTNode & { kind: 'transformation' }, options?: ASTToSchemaAsyncOptions): any {
  const { type, message } = ast;

  // Check for custom transformation
  if ('customKey' in ast && ast.customKey) {
    const customImpl = options?.transformationDictionary?.get(ast.customKey);
    if (!customImpl) {
      throw new Error(
        `Custom transformation '${ast.customKey}' referenced but not found in transformation dictionary. ` +
        `Provide the implementation in options.transformationDictionary.`
      );
    }
