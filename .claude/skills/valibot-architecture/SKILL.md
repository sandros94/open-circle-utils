---
name: valibot-architecture
description: Use this guide when library authors and advanced users want to understand how Valibot works under the hood. It covers the internal object model — schemas, actions, datasets, issues, and config — and how they fit together in the pipeline execution engine.
license: MIT
metadata:
  author: open-circle
  version: "1.0"
...
Schemas are the starting point for using Valibot. They validate a specific data type, like a string, object, or date, and can be reused or nested to reflect more complex data structures. Every schema is a plain object that satisfies [`BaseSchema`](https://valibot.dev/api/BaseSchema/):
...
Validation logic beyond the base type check lives in a `pipe` array added by the [`pipe`](https://valibot.dev/api/pipe/) method and some schemas expose additional schema-specific properties. See [Runtime properties](../integrate-valibot) for a full breakdown.

Any object that satisfies the `BaseSchema` interface is a valid schema — whether it comes from Valibot's built-ins, a third-party package, or your own code. The guide [Extend Valibot](../extend-valibot/) walks through building one from scratch.

## Actions

Actions come in three kinds. The first and probably most important one are validation actions. They check an already-typed value and may add issues. Every validation action is a plain object that satisfies [`BaseValidation`](https://valibot.dev/api/BaseValidation/):
...
The second one are transformation actions. They convert the value to a new type and/or value. Every transformation action is a plain object that satisfies [`BaseTransformation`](https://valibot.dev/api/BaseTransformation/):
...
The third one are metadata actions. They carry static annotations and are always skipped during pipeline execution. Every metadata action is a plain object that satisfies [`BaseMetadata`](https://valibot.dev/api/BaseMetadata/):
...
When a schema or validation action finds a problem with the input, it adds an issue to the dataset. Every issue is a plain object that satisfies [`BaseIssue`](https://valibot.dev/api/BaseIssue/):
...
`BaseIssue` also extends [`Config`](https://valibot.dev/api/Config/), so the `lang`, `message`, `abortEarly`, and `abortPipeEarly` fields from the parse config are carried into the issue object as well.

## Config

Every `'~run'` call receives a config object alongside the dataset. It controls language selection, custom error messages, and early-abort behavior. The [`Config`](https://valibot.dev/api/Config/) interface has four fields:
...
The [`pipe`](https://valibot.dev/api/pipe/) method is the universal connector between all building blocks. It returns a new schema object that spreads all properties of the root schema and adds a `pipe` property — a tuple with the root schema at index 0 and additional pipe items at index 1+.
...
When we need a modified copy of a schema, we spread it into a new object and replace only the properties we want to change. Here is a simplified version of our [`fallback`](https://valibot.dev/api/fallback/) method to demonstrate this pattern:
...
function fallback(schema, fallbackValue) {
  return {
    ...
    "~run"(dataset, config) {
      ...
      return outputDataset.issues ? { typed: true, value: fallbackValue } : outputDataset;
    },
  };
}
```
...
If you want to create an entirely new schema or action from scratch rather than wrapping an existing one, see the [Extend Valibot](../extend-valibot/) guide.
