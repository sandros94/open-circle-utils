...
describe("resolve-input", () => {
  describe("isValibotSchema", () => {
    ...
    test("returns false for a SchemaToASTResult", () => {
      const result = schemaToAST(v.string());
      expect(isValibotSchema(result)).toBe(false);
    });
  ...
  describe("isSchemaToASTResult", () => {
    test("returns true for a SchemaToASTResult", () => {
      const result = schemaToAST(v.string());
      expect(isSchemaToASTResult(result)).toBe(true);
    });

    test("returns false for an ASTDocument", () => {
      const doc = schemaToAST(v.string()).document;
      expect(isSchemaToASTResult(doc)).toBe(false);
    });

    test("returns false for a Valibot schema", () => {
      expect(isSchemaToASTResult(v.string())).toBe(false);
    });

    test("returns false for primitives", () => {
      expect(isSchemaToASTResult(null)).toBe(false);
      expect(isSchemaToASTResult(undefined)).toBe(false);
    });
  });
  ...
  describe("isASTNode", () => {
    ...
    test("returns true for a validation ASTNode in a pipe", () => {
      const doc = schemaToAST(v.pipe(v.string(), v.email())).document;
    ...
    test("returns false for a SchemaToASTResult", () => {
      const result = schemaToAST(v.string());
      expect(isASTNode(result)).toBe(false);
    });
  ...
  describe("resolveInput", () => {
    ...
    test("SchemaToASTResult → extracts .document.schema", () => {
      const astResult = schemaToAST(v.number());
      const node = resolveInput(astResult);
      expect(node.kind).toBe("schema");
      expect(node.type).toBe("number");
    });

    test("ASTDocument → extracts .schema", () => {
      const doc = schemaToAST(v.number()).document;
    ...
    test("ASTNode → returned as-is", () => {
      const original = schemaToAST(v.boolean()).document.schema;
